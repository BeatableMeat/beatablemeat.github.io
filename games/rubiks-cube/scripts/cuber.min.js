...
(function () {
    ...
    THREE.CSS3DRenderer = function () {
        ...
        this.setClearColor = function () { }; this.setSize = function (e, g) { a = e; b = g; c = a / 2; d = b / 2; f.style.width = e + "px"; f.style.height = g + "px"; h.style.width = e + "px"; h.style.height = g + "px" }; var g = function (a) { return 1E-6 > Math.abs(a) ? 0 : a }, m = function () {
            var a = new THREE.Vector3, b = new THREE.Vector3, c = new THREE.Euler, e = new THREE.Quaternion; c._quaternion = e; e._euler = c; return function (d) {
                d.decompose(a, e, b); return "translate3d(-50%,-50%,0) translate3d(" + g(a.x) + "px, " + g(a.y) + "px, " + g(a.z) + "px) rotateX(" + g(c.x) + "rad) rotateY(" + g(c.y) +
            ...
            if (a instanceof THREE.CSS3DObject) {
                var c; a instanceof THREE.CSS3DSprite ? (e.copy(b.matrixWorldInverse), e.transpose(), e.copyPosition(a.matrixWorld), e.scale(a.scale), e.elements[3] = 0, e.elements[7] = 0, e.elements[11] = 0, e.elements[15] = 1, c = m(e)) : c = m(a.matrixWorld); var d = a.element; d.style.WebkitTransformStyle = "preserve-3d"; d.style.MozTransformStyle = "preserve-3d"; d.style.oTransformStyle = "preserve-3d"; d.style.transformStyle =
                    "preserve-3d"; d.style.WebkitTransform = c; d.style.MozTransform = c; d.style.oTransform = c; d.style.transform = c; d.parentNode !== h && h.appendChild(d)
            } c = 0; for (d = a.children.length; c < d; c++)s(a.children[c], b)
            ...
            var k = "translate3d(0,0," + k + "px)", l; l = e.matrixWorldInverse.elements; l = "matrix3d(" + g(l[0]) + "," + g(-l[1]) + "," + g(l[2]) + "," + g(l[3]) + "," + g(l[4]) + "," + g(-l[5]) + "," + g(l[6]) + "," + g(l[7]) + "," + g(l[8]) + "," + g(-l[9]) + "," + g(l[10]) + "," + g(l[11]) + "," + g(l[12]) + "," + g(-l[13]) + "," + g(l[14]) + "," + g(l[15]) + ")"; k = k + l + " translate3d(" + c + "px," + d + "px, 0)"; h.style.WebkitTransform = k; h.style.MozTransform = k; h.style.oTransform = k; h.style.transform = k; s(a, e)
        }
    ...
    ERNO.Twist.validate = function () {
        var a = Array.prototype.slice.call(arguments), b, c, d, e, f, h; for (c = 0; c < a.length; c++)if (b = a[c], d = c + 1 < a.length ? a[c + 1] : void 0, !(b instanceof ERNO.Twist)) if ("string" === typeof b) if (1 === b.length) a[c] = "number" === typeof d ? new ERNO.Twist(b, d) : new ERNO.Twist(b); else {
            if (1 < b.length) {
                d = /(-?\d+|[XLMRYUEDZFSB])/gi; b = b.match(d); for (f = 0; f < b.length; f++)e = b[f], _.isNumeric(e) ? b[f] = +e : (d = b.slice(0, f), h = b.slice(f + 1), e = e.split(""), b = d.concat(e, h)); d = a.slice(0, c); h = a.slice(c + 1); a = d.concat(b, h);
                c--
            }
        } else b instanceof ERNO.Direction ? a[c] = b.initial : b instanceof Array ? (d = a.slice(0, c), h = a.slice(c + 1), a = d.concat(b, h)) : a.splice(c, 1), c--; return a
    ...
    this.axis = new THREE.Vector3; this.invertedAxis = new THREE.Vector3; this.matrix = new THREE.Matrix4; this.axis.rotation = 0; this.indices = a; this.neighbour = null; this.ableToHideInternalFaces = !0; this.cube = b; this.getCubelet = function (c) { return b.cubelets[a[c]] }; this.rotateGroupMappingOnAxis = function () {
        var c = new THREE.Vector3, d = new THREE.Vector3(1, 1, 1), e = new THREE.Vector3, f = new THREE.Vector3, h = new THREE.Matrix4, g; return function (m) {
            m = Math.round(m / (0.25 * Math.PI)) * Math.PI * 0.25; c.copy(d); c.sub(this.axis);
            var s = b.cubelets.slice(); h.makeRotationAxis(this.axis, -1 * m); for (var q = a.length, p; 0 < q--;)p = b.cubelets[a[q]], e.set(p.addressX, p.addressY, p.addressZ), f.copy(e), e.multiply(c).applyMatrix4(h), e.x = Math.round(e.x), e.y = Math.round(e.y), e.z = Math.round(e.z), e.add(f.multiply(this.axis)), e.add(d), e.y = 2 - e.y, e.z = 2 - e.z, b.cubelets[p.address] = s[9 * e.z + 3 * e.y + e.x]; for (q = 0; q < b.cubelets.length; q++)b.cubelets[q].setAddress(q); h.makeRotationAxis(this.axis, m); this.cubelets.forEach(function (a) {
                g = []; a.faces.forEach(function (a,
                    b) { e.copy(ERNO.Direction.getDirectionByName(a.normal).normal); e.applyMatrix4(h); g[ERNO.Direction.getDirectionByNormal(e).id] = a; a.normal = ERNO.Direction.getDirectionByNormal(e).name }); a.faces = g.slice(); a.front = a.faces[0]; a.up = a.faces[1]; a.right = a.faces[2]; a.down = a.faces[3]; a.left = a.faces[4]; a.back = a.faces[5]
            })
        }
    }(); this.map()
    ...
    ERNO.Fold.prototype.getText = function () { var a = ""; this.map.forEach(function (b) { a += b.innerHTML }); return a }; ERNO.Fold.prototype.setText = function (a) { var b; a = a.justifyLeft(18); for (b = 0; 18 > b; b++)this.map[b].innerHTML = a.substr(b, 1) }; ERNO.Projector = function () {
        return function (a, b) {
            function c(a, b) { l.getInverse(b.projectionMatrix); f.multiplyMatrices(b.matrixWorld, l); return a.applyProjection(f) } function d(d, f, l) {
                var t = b, k = t !== document ? t.getBoundingClientRect() : { left: 0, top: 0, width: window.innerWidth, height: window.innerHeight }; t !== document && (t = t.ownerDocument.documentElement, k.left += window.pageXOffset - t.clientLeft, k.top += window.pageYOffset - t.clientTop); e = k; g.x = (f - e.left) / e.width * 2 - 1; g.y = (l - e.top) / e.height * -2 + 1; g.z = -1; m.set(g.x, g.y, 1);
                c(g, d); c(m, d); m.sub(g).normalize(); q.set(g, m); h.getInverse(a.matrixWorld); q.applyMatrix4(h); return q
            } var e, f = new THREE.Matrix4, h = new THREE.Matrix4, g = new THREE.Vector3, m = new THREE.Vector3(1, 1, 1), s = new THREE.Vector3, q = new THREE.Ray, p = new THREE.Box3, k = new THREE.Sphere, l = new THREE.Matrix4, u = g.distanceTo(m); p.min.set(0.5 * -a.size, 0.5 * -a.size, 0.5 * -a.size); p.max.set(0.5 * a.size, 0.5 * a.size, 0.5 * a.size); k.radius = u * a.size * 0.5; return {
                getIntersection: function (b, c, e, f, g) {
                    f = f || new THREE.Vector3; if (null === c || null ===
                        e) return null; d(b, c, e); return q.isIntersectionSphere(k) && null !== q.intersectBox(p, f) ? (g && (b = s || new THREE.Vector3, b.copy(f).set(Math.round(b.x), Math.round(b.y), Math.round(b.z)).multiplyScalar(2 / a.size).set(b.x | 0, b.y | 0, b.z | 0), g.setFromNormalAndCoplanarPoint(s, f)), f) : null
                }, getIntersectionOnPlane: function (a, b, c, e, f) { if (null === b || null === c) return null; d(a, b, c); return q.intersectPlane(e, f) }, getCubeletAtIntersection: function () {
        ...
        return function (a, b, c, d, e) {
            ...
            v.enabled && 2 !== e.button && (l = (e.touches && e.touches[0] || e).clientX, u = (e.touches && e.touches[0] || e).clientY, m.getIntersection(b, l, u, s, w) && (null !== e.touches && e.preventDefault(), 0 === a.isTweening() && (E = "undefined" !== typeof window && void 0 !== window.performance && void 0 !== window.performance.now ? window.performance.now() : Date.now(), v.active = !0, q = m.getCubeletAtIntersection(s), p = [a.slices[q.addressX + 1], a.slices[q.addressY + 4], a.slices[q.addressZ +
            ...
            } function h(a) { v.active && (x.x = (a.touches && a.touches[0] || a).clientX, x.y = (a.touches && a.touches[0] || a).clientY); v.enabled && (a.preventDefault(), a.stopImmediatePropagation()) } function g(b) {
                var e = (b.touches && b.touches[0] || b).clientX, d = (b.touches && b.touches[0] ||
                    b).clientY; v.active = !1; v.enabled && (e !== u || d !== u) && n && (null !== b.touches && b.preventDefault(), b = k.name[0].toUpperCase(), e = Math.round(C / Math.PI * 2) * Math.PI * 0.5, 0.3 < t.length() / (("undefined" !== typeof window && void 0 !== window.performance && void 0 !== window.performance.now ? window.performance.now() : Date.now()) - E) && (e = Math.floor(C / Math.PI * 2) * Math.PI * 0.5, e += 0 < D.dot(t.normalize()) ? 0.5 * Math.PI : 0), a.twist(new ERNO.Twist(b, e.radiansToDegrees()))); E = 0; x.x = void 0; x.y = void 0; n = !1; k = null; c.removeEventListener("mousemove",
                        h); c.removeEventListener("touchmove", h); c.removeEventListener("mouseup", g); c.removeEventListener("touchend", g); c.removeEventListener("touchcancel", g); c.addEventListener("mousedown", f); c.addEventListener("touchstart", f)
            } var m = new ERNO.Projector(a, c), s = new THREE.Vector3, q, p, k, l, u, A = new THREE.Vector3, n = !1, w = new THREE.Plane, t = new THREE.Vector3, D = new THREE.Vector3, x = new THREE.Vector2; new THREE.Vector3; var r = new THREE.Vector3, C = 0, E = 0; x.x = void 0; x.y = void 0; var v = {
                active: !1, enabled: !0, multiDrag: e || !1, multiDragSnapArea: 100,
                dragSpeed: d || 1.3
            }; THREE.EventDispatcher.prototype.apply(v); v.getIntersectionAt = function () { var a = new THREE.Vector3, c = new THREE.Plane; return function (e, d) { return null === m.getIntersection(b, e, d, a, c) ? null : { cubelet: m.getCubeletAtIntersection(a), face: 1 === c.normal.x ? "RIGHT" : -1 === c.normal.x ? "LEFT" : 1 === c.normal.y ? "UP" : -1 === c.normal.y ? "DOWN" : 1 === c.normal.z ? "FRONT" : "BACK" } } }(); v.update = function () {
                var c = x.x, e = x.y; v.enabled && v.active && void 0 !== c && void 0 != e && (l !== c || u !== e) && (m.getIntersectionOnPlane(b, c, e, w, A),
                ...
                var c = this.getIntersectionAt(a, b); return c ? (this.dispatchEvent(new CustomEvent("click",
                    { detail: c })), !0) : !1
            }.bind(v), y, z; c.addEventListener("mousedown", function (a) { y = a.clientX; z = a.clientY }); c.addEventListener("mouseup", function (a) { var b = a.clientX; a = a.clientY; !n && Math.abs(Math.sqrt((b - y) * (b - y) + (a - z) * (a - z))) < 10 * (window.devicePixelratio || 1) && F(y, z) }); c.addEventListener("touchstart", function (a) { y = a.touches[0].clientX; z = a.touches[0].clientY }); c.addEventListener("touchend", function (a) {
                var b = a.changedTouches[0].clientX, c = a.changedTouches[0].clientY; !n && Math.abs(Math.sqrt((b - y) * (b - y) + (c - z) *
                    (c - z))) < 10 * (window.devicePixelratio || 1) && F(y, z) && a.preventDefault()
            }); return v
        }
    }(); ERNO.Controls = function () {
        var a, b; a = 0; b = 1; return function (c, d, e) {
            function f(b) { n.enabled && 1 === b.which && null === A.getIntersection(d, b.pageX, b.pageY) && (p = a, k.multiplyScalar(0), w(b.pageX, b.pageY, l), u.copy(l), n.domElement.removeEventListener("mousedown", f), document.addEventListener("mousemove", h), document.addEventListener("mouseup", g)) } function h(b) { n.enabled && (b.preventDefault(), p = a, w(b.pageX, b.pageY, l), k.subVectors(l, u), u.copy(l)) } function g(a) {
                document.removeEventListener("mousemove", h); document.removeEventListener("mouseup",
                    g); n.domElement.addEventListener("mousedown", f); n.enabled && (p = b)
            } function m(b) { n.enabled && null === A.getIntersection(d, b.touches[0].pageX, b.touches[0].pageY) && (p = a, k.multiplyScalar(0), w(b.touches[0].pageX, b.touches[0].pageY, l), u.copy(l), n.domElement.removeEventListener("touchstart", m), document.addEventListener("touchend", q), document.addEventListener("touchmove", s)) } function s(b) { n.enabled && (p = a, w(b.changedTouches[0].pageX, b.changedTouches[0].pageY, l), k.subVectors(l, u), u.copy(l)) } function q(a) {
                document.removeEventListener("touchend",
                    q); document.removeEventListener("touchmove", s); n.domElement.addEventListener("touchstart", m); n.enabled && (p = b)
            } var p = -1, k = new THREE.Vector2, l = new THREE.Vector2; new THREE.Vector2; var u = new THREE.Vector2, A = new ERNO.Projector(c, e), n = { enabled: !0, domElement: e, rotationSpeed: 4, damping: 0.25 }, w = function (a, b, c) {
                var e; e = n.domElement; e = e !== document ? e.getBoundingClientRect() : { left: 0, top: 0, width: window.innerWidth, height: window.innerHeight }; var d = window.devicePixelRatio || 1; return c.set(0.001 * (a * d - e.width - e.left),
                    0.001 * (e.height + e.top - b * d))
            }; n.update = function () { var e = new THREE.Vector3, f = 0, g = new THREE.Matrix4; return function () { !1 !== n.enabled && -1 !== p && (e.set(k.y, -1 * k.x, 0).normalize(), g.getInverse(c.matrixWorld), g.multiply(d.matrixWorld), e.transformDirection(g), k.multiplyScalar(1 - Math.max(0, Math.min(1, n.damping))), f = k.length(), c.object3D.rotateOnAxis(e, -f * n.rotationSpeed), p === a ? p = -1 : p === b && 1E-4 <= f ? l.add(k) : p = -1) } }(); n.domElement.addEventListener("mousedown", f); n.domElement.addEventListener("touchstart", m); return n
        }
    }(); var SceneType = THREE.Scene; THREE.Scene = SceneType || function () { }; ERNO.renderers = ERNO.renderers || {};
    ERNO.renderers.CSS3D = function (a, b) {
        function c(a) { a.style.display = "block" } function d(a) { a.style.display = "none" } function e() { return Array.prototype.slice.call(h.domElement.querySelectorAll(".faceLabel")) } function f() { if (b.domElement.parentNode) { var a = b.domElement.parentNode.clientWidth, c = b.domElement.parentNode.clientHeight; !b.domElement.parentNode || b.domElement.clientWidth === a && b.domElement.clientHeight === c || b.setSize(a, c); h.render(g, b.camera) } requestAnimationFrame(f) } var h = new THREE.CSS3DRenderer,
    ...
    ERNO.renderers.CSS3DCubelet.methods = function () {
        function a(a) { a.style.display = "block" } function b(a) { a.style.display = "none" } return {
            ...
            }, showIntroverts: function () { var b = new THREE.Vector3, d = new THREE.Matrix4, e; return function (f, h) { e = ""; f && (d.getInverse(this.matrix), b.copy(f).transformDirection(d), e = 1 === Math.abs(Math.round(b.x)) ? ".axisX" : 1 === Math.round(Math.abs(b.y)) ? ".axisY" : ".axisZ"); this.getFaceElements(".faceIntroverted" + (void 0 !== f ? e : "")).forEach(a); h || (this.showingIntroverts = !0) } }(), hideIntroverts: function () {
                var a = new THREE.Vector3, d = new THREE.Matrix4, e; return function (f, h) {
                    e = ""; f && (d.getInverse(this.matrix),
                        a.copy(f).transformDirection(d), e = 1 === Math.abs(Math.round(a.x)) ? ".axisX" : 1 === Math.round(Math.abs(a.y)) ? ".axisY" : ".axisZ"); this.getFaceElements(".faceIntroverted" + (void 0 !== f ? e : "")).forEach(b); h || (this.showingIntroverts = !1)
                }
    ...
    ERNO.extend(ERNO.Cube.prototype, {
        ...
        0.8 <= this.verbosity && console.log("Executing a twist command to rotate the " + a.group + " " + a.wise + " by", a.degrees, "degrees."); for (var b = this.slicesDictionary[a.command.toLowerCase()], c = (void 0 === a.degrees ? 90 : a.degrees) * a.vector, d = c.degreesToRadians(), e = Math.abs(d - b.rotation) / (0.5 * Math.PI) * this.twistDuration, f = b.indices.length, h; 0 < f--;)b.getCubelet(f).isTweening = !0; (new TWEEN.Tween(b)).to({ rotation: d },
            e).easing(TWEEN.Easing.Quartic.Out).onComplete(function () { b.rotation = d; b.axis.rotation = 0; for (f = b.indices.length; 0 < f--;)h = b.getCubelet(f), h.isTweening = !1, h.updateMatrix(), h.matrixSlice.copy(h.matrix); 0 !== c && (b.rotateGroupMappingOnAxis(d), this.dispatchEvent(new CustomEvent("onTwistComplete", { detail: { slice: b, twist: a } }))); a === this.finalShuffle && (this.finalShuffle = null, this.dispatchEvent(new CustomEvent("onShuffleComplete", { detail: { slice: b, twist: a } }))) }.bind(this)).start(this.time)
        }, getText: function (a) {
            if (void 0 ===
